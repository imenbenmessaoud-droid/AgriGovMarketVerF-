...
const MinistryTopBar = ({ searchQuery, onSearchChange, toggleSidebar }) => {
    ...
    <header className="h-14 bg-white border-b border-gray-100 flex items-center justify-between px-6 sticky top-0 z-40">
      ...
      <div className="flex items-center gap-6 flex-1">
        <button 
          ...
          className="p-1.5 -ml-2 hover:bg-gray-50 rounded-xl transition-all active:scale-95 text-gray-600 border border-transparent hover:border-gray-100 shadow-sm bg-white"
        ...
        <div className="flex-1 max-w-xl relative group">
          <input
            ...
            placeholder="search users, categories, prices, reports..."
            className="w-full h-9 pl-10 pr-4 bg-gray-50/50 text-gray-800 rounded-xl border border-gray-100 focus:outline-none focus:ring-2 focus:ring-blue-600/5 focus:border-blue-600/20 focus:bg-white transition-all placeholder:text-gray-400 text-sm"
          />
          <div className="absolute left-3.5 top-1/2 -translate-y-1/2 pointer-events-none">
            <FaSearch className="text-gray-300 group-focus-within:text-blue-500 transition-colors" size={13} />
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center gap-6">
        ...
        <div className="relative">
          ...
          >
            ...
            {unreadCount > 0 && (
              <span className="absolute top-[6px] right-[6px] bg-red-500 text-white text-[9px] min-w-[16px] h-[16px] px-0.5 flex items-center justify-center rounded-full border border-white font-semibold shadow-sm pointer-events-none">
                {unreadCount}
              </span>
            )}
          </button>
            ...
            >
              <div className="p-4 border-b border-gray-50 flex justify-between items-center bg-gray-50/50">
                ...
                <button
                  ...
                  className="text-[11px] text-blue-600 font-normal hover:text-blue-700 transition-colors"
                >
                  Mark all as read
                </button>
              </div>
        ...
        <div className="flex items-center gap-3 cursor-pointer pl-4 border-l border-gray-200 user-icon-button" onClick={() => setIsSidebarOpen(true)}>
          <div className="flex flex-col items-end hidden sm:flex">
            <p className="text-sm font-medium text-gray-900 leading-tight">Ministry</p>
            <p className="text-xs text-gray-500">{userProfile.wilaya.split('-')[1]?.trim() || userProfile.wilaya}</p>
          </div>
          <div className="w-10 h-10 bg-blue-800 rounded-full flex items-center justify-center text-white text-sm font-normal shadow-sm overflow-hidden">
            {userProfile.avatar ? (
            ...
            <div className="bg-gradient-to-r from-gray-800 to-gray-700 px-6 py-6 text-center relative">
              {isLoggedIn ? (
                <div className="flex flex-col items-center">
                  ...
                  <div className="inline-flex px-2 py-0.5 bg-blue-500/20 text-blue-300 rounded-full text-[9px] font-normal mt-1 uppercase tracking-wider">
                    {userProfile.role}
                  </div>
                </div>
              ) : (
